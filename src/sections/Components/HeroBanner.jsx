...
// 🔥 Función para optimizar imágenes de Cloudinary a WebP automáticamente
...
export default function HeroBanner({ image }) {
  const [bannerImage, setBannerImage] = useState(getOptimizedImage(image) || null);
    ...
    const fetchBanner = async () => {
      try {
        ...
        if (data?.imageUrl) {
          ...
          setBannerImage(getOptimizedImage(image) || null);
        }
      } catch (err) {
  ...
  const finalImage = getOptimizedImage(preview || bannerImage || image);
